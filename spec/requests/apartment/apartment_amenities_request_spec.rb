...
RSpec.describe 'Amenity', type: :request do
  ...
  describe 'POST /api/v2/apartments/:apartment_id/apartment_amenities' do
    let(:valid_params) { { amenity_id: create(:amenity).id } }

    context 'with valid params' do
      before do
        post "/api/v2/apartments/#{apartment.id}/apartment_amenities", params: valid_params.to_json, headers: headers
      end
  ...
  describe 'DELETE /apartments/:apartment_id/apartment_amenities/:id' do
    context 'with valid apartment ID' do
      it 'deletes the apartment amenity' do
        delete "/api/v2/apartments/#{apartment.id}/apartment_amenities/#{apartment_amenity.id}", headers: headers
    ...
    context 'with invalid apartment ID' do
      it 'deletes the apartment' do
        delete '/api/v2/apartments/1000/apartment_amenities/1', headers: headers
        expect(JSON.parse(response.body)['error']).to eq('Record Not Found')
      end
    end
  end

...
RSpec.describe 'Comment', type: :request do
  ...
  describe 'GET /api/v1/apartments/:apartment_id/comments' do
    before do
      create_list(:comment, 4, apartment: apartment)
    end

    it 'returns success code' do
      get "/api/v1/apartments/#{apartment.id}/comments"
    ...
    it 'returns 404 status for an invalid id' do
      get '/api/v1/apartments/1000/comments'
  ...
  describe 'POST /api/v1/apartments/:apartment_id/comments' do
    let(:valid_params) { { content: 'Comment' } }

    context 'with valid params' do
      before do
        post "/api/v1/apartments/#{apartment.id}/comments", params: valid_params.to_json, headers: headers
      end
  ...
  describe 'PUT /api/v1/apartments/:apartment_id/comments/:id' do
    context 'with valid params' do
      ...
      it 'updates the existing record' do
        put "/api/v1/apartments/#{apartment.id}/comments/#{comment.id}", params: comment_attrs.to_json, headers: headers
  ...
  describe 'DELETE /apartments/:id' do
    ...
    context 'with invalid apartment ID' do
      it 'deletes the apartment' do
        delete '/api/v1/apartments/1000/comments/1', headers: headers
        expect(JSON.parse(response.body)['error']).to eq('Record Not Found')
      end
    end
  end

...
RSpec.describe BookingApartmentPolicy, type: :policy do
  ...
  let(:booking_apartment) { create(:booking_apartment, apartment: apartment, user: user) }
  ...
  permissions :update? do
    context 'when auth user is a host' do
      it 'grants access if user\'s apartment' do
        expect(subject).to permit(user, booking_apartment)
      end

      it 'deny if not user\'s comment' do
        expect(subject).not_to permit(host, booking_apartment)
      end
    end
